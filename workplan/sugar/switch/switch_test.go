...
func (d *dummyNode) Run(_ context.Context, _ *types.WorkflowContext) (string, error) {
	return `""`, nil
}

func addDummy(g *coreplan.Plan, id string) {
...
func TestIf_EdgeLabels(t *testing.T) {
	...
	if m["true"] != "t" {
		t.Errorf("true label: %v", m)
	}
	if m["false"] != "f" {
		t.Errorf("false label: %v", m)
	}
}
...
func TestIf_ConditionEvaluation(t *testing.T) {
	...
	for _, e := range edges {
		if e.Condition != nil {
			if e.Label == "true" && !e.Condition(wc) {
				t.Error("true cond should match")
			}
			if e.Label == "false" && e.Condition(wc) {
				t.Error("false cond should not match")
			}
		}
	}
}
...
func TestSwitch_EdgeCount(t *testing.T) {
	...
	if len(g.GetEdgesFrom("sw3")) != 3 {
		t.Error("expected 3 edges")
	}
}
...
func TestSwitch_DefaultCase(t *testing.T) {
	...
	for _, e := range edges {
		if e.To == "d" && e.Condition != nil && e.Condition(wc) {
			return
		}
	}
...
func TestContains(t *testing.T) {
	if !Contains("hello")("say hello world") {
...
func TestNotContains(t *testing.T) {
	if !NotContains("error")("success") {
		t.Error("should match")
	}
	if NotContains("error")("error occurred") {
		t.Error("should not match")
	}
}

func TestNotContains_Empty(t *testing.T) {
	if !NotContains("")("anything") {
		t.Error("empty should match")
	}
}

func TestContainsAndNotContainsAreInverses(t *testing.T) {
	if Contains("x")("x") == NotContains("x")("x") {
		t.Error("should be inverses")
	}
}
