...
// 编译期断言：*Holder 实现了 seelectx.ToolDispatcher。
var _ seelectx.ToolDispatcher = (*Holder)(nil)
...
func (h *Holder) Dispatch(ctx context.Context, name, argsJSON string) (string, error) {
	...
	for attempt := 0; attempt < h.DispatchRetries; attempt++ {
		...
		lastErr = err
		log.Printf("[tool.dispatch] tool %q UNAVAILABLE, retry %d/%d: %v",
