...
func (h *Holder) chatLoop(ctx context.Context, userInput string, strategy completionStrategy) (string, error) {
	...
	for loop := 0; loop < h.cfg.MaxLoops; loop++ {
		...
		if h.lastCompressLoop < 0 || loop-h.lastCompressLoop > 1 {
			if history.NeedCompression(h.history, cfg.CompressThreshold) {
				...
				if err != nil {
					log.Printf("[session.chatLoop] %s compression failed: %v, using hard trim", h.sessionID, err)
	...
	return "", fmt.Errorf("[session] %s reached maxLoops (%d) without a final text reply",
